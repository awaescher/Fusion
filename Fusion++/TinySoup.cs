	...
	public class WebSoupClient : ISoupClient
	{
		...
		internal async Task<IList<AvailableVersion>> CallWebServiceAsync(string method, string parameterString)
		{
			try
			{
				...
				var proxyUri = new Uri(WebRequest.DefaultWebProxy.GetProxy(targetUri).AbsoluteUri);
				var directAccess = proxyUri?.Authority?.Equals(targetUri.Authority) == true;
		...
		public string ShortestVersionString
		{
			get
			{
				return Version.Major.ToString()
					...
					+ (Version.Build > 0 || Version.Revision > 0 ? "." + Version.Build.ToString() : "")
